...
- Use additive analysis reports for compatibility visibility: `MORVA5001` is a non-fatal container warning, while existing `check()` and simulator APIs remain error-only. Do not infer severity from message text.
